...
|---|---|---|---|---|
| Public blogs | `getPublicBlogsDB()` loads every published blog, then `/blogs` slices the array in application memory for pagination. | Database rows and serialized server data grow linearly with blog count on every cache miss; memory and response preparation are unbounded. | Medium | Push `skip`/`take` and a count (or cursor) into the database, or use a bounded “featured + page” query. |
